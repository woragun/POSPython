...
class FMS:
    def __init__(self,root):
        ...
        self.home_img = self.home_img.resize((250,250))
        self.home_img = ImageTk.PhotoImage((self.home_img))
        
        self.worker_img = Image.open('img\worker.png')
        self.worker_img = self.worker_img.resize((250,250))
        self.worker_img = ImageTk.PhotoImage((self.worker_img))

        self.storage_img = Image.open('img\storage.png')
        self.storage_img = self.storage_img.resize((250,250))
        ...
        btn_home = Button(Menu,image =self.home_img,bg = '#517d30',highlightthickness = 0, bd = 0).pack(side=TOP,pady=40)
        btn_worker = Button(Menu,image=self.worker_img,command=self.worker,bg = '#517d30',highlightthickness=0,bd=0).pack(side=TOP,pady=40)
        btn_storage = Button(Menu,image=self.storage_img,command=self.stock,bg = '#517d30',highlightthickness=0,bd=0).pack(side=TOP,pady=40)
